    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    String skaterId = skaterManager.FindSkater(model.Email);
                    if (skaterManager.FindSkater(model.Email) != "")
                    {
                        SkaterVM oldSkater = skaterManager.LoginSkater(skaterId, model.Password);
                        var user = new ApplicationUser
                        {
                            UserName = oldSkater.Email,
                            GivenName = oldSkater.GivenName,
                            FamilyName = oldSkater.FamilyName,
                            Email = oldSkater.Email,
                            SkaterID = oldSkater.SkaterID,
                            //other fields here
                        ...
                        if (result.Succeeded)
                        {
                            foreach (var role in oldSkater.Roles)
                            {
                    ...
                    else //not an existing user
                    {
                        ...
                        string _passwordhash = sm.hashSHA256(model.Password);
                        SkaterVM skater = new SkaterVM()
                        {


                            Email = model.Email,
                            SkaterID = model.SkaterID,
        ...
        public async Task<ActionResult> RegisterEmployeeUser(RegisterEmployeeViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                LogicLayer.SkaterManager usrMgr = new LogicLayer.SkaterManager();
                try
                {
                    if (usrMgr.FindSkater(model.Email) != "")
                    {
                        // if this user exists, we need to use the regular register method
                        return RedirectToAction("Register", "Account");
                    }

                    else
                    { //not an existing user
                        var employee = new DataObjects.SkaterVM()
                        {
                            Email = model.Email,
                            GivenName = model.GivenName,
                            FamilyName = model.FamilyName,
                        ...
                        if (usrMgr.addSkater(employee) == 1)
                        {
                            string skaterID = usrMgr.getSkaterByEmail(model.Email).SkaterID;
                            var user = new ApplicationUser
                            {
                                SkaterID = skaterID,
                                GivenName = model.GivenName,
